...
|---|---|---|
| `==` | `bool(a == b)` | Equality. `bool` is a subclass of `int` in Python, so `True == 1` is `True` and `False == 0` is `True`; the TS port MUST special-case bool↔number coercion (`Number(true) === 1`) without pulling in the JS `==` hazards (`"" == 0`, `null == undefined`). |
| `!=` | `bool(a != b)` | Inequality. (Same bool↔int coercion rule as `==`.) |
...
- `verdict_signature` (replay guard) is HMAC-SHA256 over
  ...
  **Missing/empty signature when a key is configured = failed
  verification** (council 2026-07-22): a network middleman that strips
  the field cannot make verification silently pass. The `risk_score`
  canonical form is the integer-valued-or-decimal string produced by
  Python `str(float)` / JS `Number.toString` — both languages agree
  on the default repr for finite floats in [0.0, 1.0] at v1.0 (no
  `1.0` vs `1` divergence because `risk_score` is typed `float` on the
  Python side and `number` on the TS side; the proto wire encoding
  is `double` so both sides receive the same IEEE 754 bytes). A
  future contract revision that introduces a different float
  canonicalization MUST bump `AuditEvent.schema_version`.
